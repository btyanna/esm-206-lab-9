...
# there is no uncertaintly in relationship between price/sqft and price because it's derived from it :O
# there is a problem with this model

# b. Model summary:
summary(homes_lm1)
# bedrooms, bathrooms, sqft all say about the size of houses
...
# in comparison to the houses in arroyo grande, houses in atascadero will be about $106333.12 more
# this model is highly sigificantly better than a model w/o any other variables (p < 0.001)
# 53% of variance price is defined by these variables (R^2)
```
...
# fct_relevel() function in forcats ..... ?fct_relevel in console
...
# puts regular first in the levels which will be the reference variable
...
# a. Model diagnostics:
plot(homes_lm3)
# plot 1, it's clear that for almost all the observations, it spread is pretty even. ignore the 3 outlier points
# since dataset is large, do not worry about heteroelasicity
```
...
fin_aic = AIC(homes_lm3)

# not needed to put in report, but is valuable when comparing multiple models

# Answer: which would you pick? 
# the first one :O ugh im not really payin attention ahhhh poooo :| :[ :( 
...
# results = 'asis' makes it into a table that will be shown through html
